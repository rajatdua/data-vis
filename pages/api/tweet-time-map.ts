...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const tweetTimeMapQuery = `SELECT time_after, time_before FROM tweets WHERE date >= ${processedStart} AND date <= ${processedEnd} AND time_after BETWEEN 2 AND 80000 AND time_before BETWEEN 2 AND 80000;`;
